...
class _AdminShippedOrdersState extends State<AdminShippedOrders> {
  ...
  Future<void> markAsDelivered(
    ...
    try {
      await orderDoc.reference.update({
        ...
        'deliveredAt': Timestamp.now(), // ✅ Delivered date/time
      });
  ...
  ...orderData,
  'status': 'delivered',           // ✅ yahan bhi delivered karo
  'deliveredAt': Timestamp.now(),  // ✅ delivered time
  'orderId': docId,
  'items': itemsWithProductId,
};


      // ✅ Update the order inside "orders" collection
...
// final status = orderData['status'] ?? "no status";
