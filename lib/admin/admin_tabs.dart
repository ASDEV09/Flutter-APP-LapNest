...
class AdminOrderTabsWidget extends StatelessWidget {
    ...
    switch (index) {
      case 0:
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (_) => const AdminOrdersPage()), // Pending Orders page
        );
        break;
      case 1:
        Navigator.pushReplacement(
  ...
  /// Firestore se count fetch karne ka function (status ke basis par)
      ...
      child: Padding(
        ...
        child: Row(
          children: List.generate(tabs.length, (index) {
            ...
            // 🔥 Cancel ke liye alag collection
            final Stream<int> countStream = (index == 3)
                ? _getCount(isCancel: true)
                : (index == 0)
                    ? _getCount(status: 'pending') // Pending
                    : (index == 1)
                        ? _getCount(status: 'shipped') // Shipped
                        : _getCount(status: 'delivered'); // Delivered
